<script setup>
  ...
  function submitRequest() {
    axios({
        method: 'get',
        url: `https://34.27.198.255/find?collectionid=${newRequest.collectionid}`,
        headers: {
...
<div class="container">
  ...
  <template v-if="newRequest.collectionid">
    <div>POST http://microdb.fredy.dev/find?collectionid={{newRequest.collectionid}} HTTP/1.1</div>
    Content-Type: application/json<br>
    
    <pre>{<br>{{newRequest._id}}<br>}</pre>
  </template>
  <h3>response:</h3>
