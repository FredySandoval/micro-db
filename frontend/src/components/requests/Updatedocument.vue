<script setup>
  ...
  function submitRequest() {
    axios({
        method: 'patch',
        url: `https://34.27.198.255/update?collectionid=${newRequest.collectionid}&_id=${newRequest._id}`,
        headers: {
...
<div class="container">
  ...
  <span class="c3label2">documentid:</span>
  <InputText class="c3input2" type="text" v-model="newRequest._id" placeholder="The document id"/>
  ...
  <template v-if="newRequest.collectionid">
    <div>POST http://microdb.fredy.dev/find?collectionid={{newRequest.collectionid}}&_id={{newRequest._id}} HTTP/1.1</div>
